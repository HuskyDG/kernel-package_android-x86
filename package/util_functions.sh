...
get_oroot_var(){
SRC=$(grep_cmdline SRC)
if [ "$BOOTMODE" == true ]; then
MAGISKTMP="$(magisk --path)"
OROOT=$MAGISKTMP/.magisk/mirror/android
# fail back to gearroot
[ ! -d "$OROOT" ] && { ui_print "- WARNING: initrd-magisk is not installed"; OROOT="/gearlock/gearroot/$SRC"; }
else
    ui_print "- Flashing from Recovery is not recommended"
    OROOT=/mnt/$SRC
fi
OROOT="$(readlink -f "$OROOT")"
[ ! -d "$OROOT" ] && abort "! Unable to find Android x86 directory"
ui_print "- Target Android directory: $OROOT"
